...
Pay::Pay()
{
    ...
    //定时查询支付状态
    payQueryTime  = new QTimer();
    connect(payQueryTime,SIGNAL(timeout()),this,SLOT(query()));
}
...
void Pay::On_alipayQRcode()
{
    ...
    if(!data.isNull() || !data.isEmpty() )
    {
        ...
        {
            emit payStatus("alipayQRCode",QString(data.mid(1,data.length()-2)));
            payQueryTime->start(2000);//2秒/次查

        }
        else
        {
            payQueryTime->stop();
            emit payStatus("error","服务器异常，获取支付宝二维码失败");
        }
    }
...
void Pay::On_wxQRcode()
{
    ...
    if(!data.isNull() || !data.isEmpty())
    {
        if(data.contains("weixin"))
          {
             payQueryTime->start(2000);//2秒/次查询
             emit payStatus("weixinQRCode",QString(data.mid(1,data.length()-2)));
        }
        else
        {
            payQueryTime->stop();
            emit payStatus("error","服务器异常，获取微信二维码失败");
        }
    }
}
void Pay::On_payStatus()
{
    ...
    if(!data.isEmpty())
    {
        ...
        {
            payQueryTime->stop();
            emit payStatus("payStatus","支付成功");
        }
        else if(ret  == "0" )emit payStatus("payStatus","支付失败");
        else
        {
           payQueryTime->stop();
           emit payStatus("error","服务器异常，查询订单失败");
        }
    }
}
